...
class Siropu_AdsManager_Route_PrefixAdmin_Transactions implements XenForo_Route_Interface
{
	public function match($routePath, Zend_Controller_Request_Http $request, XenForo_Router $router)
	{
		$action = $router->resolveActionWithIntegerParam($routePath, $request, 'transaction_id');
		return $router->getRouteMatch('Siropu_AdsManager_ControllerAdmin_Transactions', $action, 'siropu_ads_manager_tran');
	}
	public function buildLink($originalPrefix, $outputPrefix, $action, $extension, $data, array &$extraParams)
	{
		return XenForo_Link::buildBasicLinkWithIntegerParam($outputPrefix, $action, $extension, $data, 'transaction_id');
	}
}