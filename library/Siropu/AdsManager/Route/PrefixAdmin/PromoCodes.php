...
class Siropu_AdsManager_Route_PrefixAdmin_PromoCodes implements XenForo_Route_Interface
{
	public function match($routePath, Zend_Controller_Request_Http $request, XenForo_Router $router)
	{
		$action = $router->resolveActionWithIntegerParam($routePath, $request, 'code_id');
		return $router->getRouteMatch('Siropu_AdsManager_ControllerAdmin_PromoCodes', $action, 'siropu_ads_manager_pcd');
	}
	public function buildLink($originalPrefix, $outputPrefix, $action, $extension, $data, array &$extraParams)
	{
		return XenForo_Link::buildBasicLinkWithIntegerParam($outputPrefix, $action, $extension, $data, 'code_id');
	}
}