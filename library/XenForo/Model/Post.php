...
class XenForo_Model_Post extends XenForo_Model
{
	...
	public function alertQuotedMembers(array $post, array $thread, array $forum)
	{
		...
		if (preg_match_all('/\[quote=("|\'|)([^,]+),\s*post:\s*(\d+?).*\\1\]/siU', $post['message'], $quotes))
		{
			...
			$quotedPostIds = array_unique($quotes[3]);
			if ($quotedPostIds)
			{
				$db = $this->_getDb();
				...
				$userIds = $db->fetchCol("
					SELECT user_id
					FROM xf_post
					WHERE post_id IN (" . $db->quote($quotedPostIds) . ")
				");
			...
			foreach ($quotes[0] AS $i => $quoteText)
			{
				...
				{
					$expectedUserIds[$quoteTextMatch[1]] = $quoteTextMatch[1];
				}
			}

			foreach ($users AS $userId => $user)
			{
				if (isset($quotedUsers[$userId])
					|| $userId == $post['user_id']
					|| !isset($expectedUserIds[$userId])
				)
				{
					continue;
				}
