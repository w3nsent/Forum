...
class XenForo_Model_Warning extends XenForo_Model
{
	...
	public function prepareWarningConversationInLanguage(array $warning, $languageId = null)
	{
		...
		$phrases = $phraseModel->getPhraseTextFromPhraseTitles(array($titlePhrase, $messagePhrase), $languageId);

		$warning['conversationTitle'] = isset($phrases[$titlePhrase]) ? $phrases[$titlePhrase] : '';
		$warning['conversationMessage'] = isset($phrases[$messagePhrase]) ? $phrases[$messagePhrase] : '';
		...
		switch ($action['action'])
		{
			case 'ban':
				...
				if ($action['action_length_type'] == 'points')
				{
					...
					{
						$reason = new XenForo_Phrase('warning_ban_reason');
						$reason = utf8_substr($reason->render(), 0, 255);
