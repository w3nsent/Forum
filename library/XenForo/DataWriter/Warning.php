	...
	*/
	...
	protected function _preSave()
	{
		...
		if ($this->get('expiry_date') > 0 AND $this->get('expiry_date') <= XenForo_Application::$time)
		{
			if ($this->isInsert())
			{
				// Can't insert an already expired warning as this won't necessarily trigger all of the
				// related behaviors. Instead, set a short expiration.
				$this->set('expiry_date', XenForo_Application::$time + 3600);
			}
			else
			{
		...
		if ($this->isInsert() || ($this->get('is_expired') == 0 && $this->getExisting('is_expired') == 1))
		{
			...
			if ($warningHandler)
			{
				...
				if ($content)
				{
					...
					$warningHandler->warn($this->getMergedData() + $user, $content,
