...
class Brivium_BriviumHelper_Model_ListenerClass extends XenForo_Model
{
	...
	public function rebuildBriviumAddOnsCache()
	{
		...
		if($addOns){
			...
			$this->_getDataRegistryModel()->delete('brBriviumAddOns');
		}
	...
	public function rebuildListenerClassCodeListener($listenerClasses = array())
	{
		...
		if($listenerClasses){
			...
			if(empty($events['front_controller_pre_route']['frontControllerPreRoute'])){
				$rows[] = "('front_controller_pre_route', 11, '', 'Brivium_BriviumHelper_EventListeners', 'frontControllerPreRoute', 1)";
			}else{
				unset($events['front_controller_pre_route']['frontControllerPreRoute']);
			}
