...
class Audentio_UIX_Model_Node extends XFCP_Audentio_UIX_Model_Node
{
    ...
    public function rebuildNodeStylingCache()
    {
        ...
        foreach ($nodes as $node) {
			...
			if ($node['inheritStyling'] && $node['parent_node_id']) {
				$parentNode = @unserialize($nodes[$node['parent_node_id']]['uix_styling']);
				if ($parentNode) {
					$nodeStylingCache[$node['node_id']] = $parentNode;
				}
			}
        }
    ...
    public function getNodeIcon($nodeId, &$nodes)
    {
        ...
        if (array_key_exists('icons', $node)) {
            ...
            if (!$xenOptions->uix_disableNodeIconInheritance) {
                ...
                if (!$this->_checkNodeIconValues($icons)) {
	...
	protected function _checkNodeIconValues($icons)
	{
		if (!$icons['category_read'] && !$icons['category_unread'] && !$icons['forum_read'] && !$icons['forum_unread'] && !$icons['link_node'] && !$icons['page_node']) {
			return false;
		}
