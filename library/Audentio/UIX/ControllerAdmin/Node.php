...
class Audentio_UIX_ControllerAdmin_Node extends XFCP_Audentio_UIX_ControllerAdmin_Node
{
    ...
    public function actionUixOptions()
    {
        ...
        if ($this->isConfirmedPost()) {
            ...
            $inheritClass = $this->_input->filterSingle('inheritClass', XenForo_Input::BOOLEAN);
            $inheritStyling = $this->_input->filterSingle('inheritStyling', XenForo_Input::BOOLEAN);
            ...
            $writer->set('collapsed', ($node['node_type_id'] == 'Category' ? $collapsed : 0));
			$writer->set('inheritClass', ($node['parent_node_id'] ? $inheritClass : 0));
			$writer->set('inheritStyling', ($node['parent_node_id'] ? $inheritStyling : 0));
