...
!function($, window, document, _undefined)
{
	...
	XenForo.FilterList.prototype =
	{
		...
		{
			var action = XenForo.canonicalizeUrl(this.$form.attr('action')),
				match;
			action = action.replace(XenForo.baseUrl(), '');
			...
			return 'FilterList_' + encodeURIComponent(action).replace(/[\.\+\/]|(%([0-9a-f]{2}|u\d+))/gi, '');
		},
	...
	XenForo.FilterListItem.prototype =
	{
		...
		filter: function(filterRegex)
		{
			...
			{
				this.$textContainer.html(XenForo.htmlspecialchars(this.text).replace(filterRegex, '<strong>$1</strong>'));
