...
uix.checkRadius = {
    ...
    resize: function() {
        if (uix.checkRadius.uixWrapper.length) {
...
uix.dateHelper = {
	...
	makeString: function(tInit) {
		var ud = uix.dateHelper,
			tStamp = Math.abs(tInit),
		...
		if (!isNow) {
			ret = ud.processPhrase(ret, tInit == tStamp);
		}
	...
	processPhrase: function(str, ago) {
		var ud = uix.dateHelper;
		if (ud.phrase.ago.indexOf('{time}') > -1) {
			if (ago) {
				return ud.phrase.ago.replace('{time}', str);
			} else {
				return ud.phrase.future.replace('{time}', str);
			}
		} else {
			return str;
		}
...
uix.debug = function(skipMarks) {
    ...
    result += uix.logVar(uix.uix_jsCacheBust, 'Javascript Cache Bust');
	result += '\n';
    ...
    result += uix.logVar(uix.nodeGridCollapseEnabled, 'uix.nodeGridCollapseEnabled');
    ...
    result += uix.logVar(uix.javascriptInitHide, 'uix.javascriptInitHide');
	result += '\n';
    ...
    result += uix.logVar(uix.stickySidebarLegacy, 'uix.stickySidebarLegacy');
    result += uix.logVar(uix.stickySidebarDisableIOS, 'uix.stickySidebarDisableIOS');
	result += '\n';
...
audentio.grid = {
    		...
    		for (var keyNum = 0, keyLen = layoutKeys.length; keyNum < keyLen; keyNum++) {
    			...
    			if (nextSubnodeId === 0) { // root node
    				...
    				if (typeof(result[keyName][subKeyName]) === 'undefined') {
                        ...
                        result[keyName][subKeyName].separator_type = keyEle.separator_type;
                        result[keyName][subKeyName].separator_width = keyEle.separator_width || 0;
    					if (keyEle.fill_last_row !== undefined && parseInt(keyEle.fill_last_row.value) >= 0) {
	...
	initGet: function(layout, minWidth) {
        ...
        if (uix.nodeGridCollapseEnabled) {
            ag.initCollapseCookie();
        }
	...
	recurse: function(ele, layoutName) {
        ...
        var parentNode = $(ele).closest('.node').get(0);
        var collapseEle = ele.getElementsByClassName('uix_nodeGridCollapseInner');
        if (collapseEle.length == 1) {
            collapseEle = collapseEle[0];
        } else {
            collapseEle = null;
        }
		...
		for (i = 0, len = children.length; i < len; i++) {
			...
			if (currentChild.tagName == 'LI') {
                ...
                child.separator_type = subEles[subLayoutName].separator_type;
                child.separator_width = subEles[subLayoutName].separator_width;
		...
		for (var subNum = 0, lenSub = subKeys.length; subNum < lenSub; subNum++) {
			...
			if (subLayoutVar.currentCols != numCols || ag.alwaysCheck) {
				...
				for (i = 0; i < childLen; i++) {
					...
					if (childEle.separator_type == 'collapse' && childEle.collapse_init !== true && (childEle.separator_width >= uix.windowWidth || childEle.separator_width == 0) && uix.nodeGridCollapseEnabled) {
						ag.eles[index].children[i].collapse_init == true;
                        eleRoot.collapseEle.appendChild(childEle.ele);
                        if (!eleRoot.collapseInit) {
                            ag.eles[index].collapseInit = true;
                            if (audentio.grid.getCollapseCookie(eleRoot.eleID)) {
                                eleRoot.parentNode.className += ' audentio_grid--expanded'
                            } else {
                                eleRoot.parentNode.className += ' audentio_grid--collapsed'
                            }
                        }
                        var collapseTrigger = eleRoot.ele.getElementsByClassName('uix_nodeGridCollapse__trigger--collapse');
                        var expandTrigger = eleRoot.ele.getElementsByClassName('uix_nodeGridCollapse__trigger--expand');

                        if (expandTrigger.length == 1) {
                            expandTrigger[0].onclick = function() {
                                var overflowWrapper = eleRoot.ele.getElementsByClassName('uix_nodeGridCollapseWrap')[0];
                                var overflowInner = eleRoot.ele.getElementsByClassName('uix_nodeGridCollapseInner')[0];

                                var targetHeight = overflowInner.offsetHeight;
                                overflowWrapper.style.maxHeight = targetHeight + 'px';
                                window.requestAnimationFrame(function() {
                                    eleRoot.parentNode.className = eleRoot.parentNode.className.replace('audentio_grid--collapsed', 'audentio_grid--expanded');
                                    ag.setCollapseCookie(eleRoot.eleID, true);
                                    window.setTimeout(function() {
                                        window.requestAnimationFrame(function() {
                                            overflowWrapper.style.transition = 'none';
                                            window.requestAnimationFrame(function() {
                                                overflowWrapper.style.maxHeight = '';
                                                window.requestAnimationFrame(function() {
                                                    overflowWrapper.style.transition = '';
                                                })
                                            })
                                        })
                                    }, 300)
                        ...
                        if (collapseTrigger.length == 1) {
                            collapseTrigger[0].onclick = function() {
                                var overflowWrapper = eleRoot.ele.getElementsByClassName('uix_nodeGridCollapseWrap')[0];
                                var overflowInner = eleRoot.ele.getElementsByClassName('uix_nodeGridCollapseInner')[0];
                                ...
                                window.requestAnimationFrame(function() {
                                    ...
                                    window.requestAnimationFrame(function() {
                                        overflowWrapper.style.transition = '';
                                        window.requestAnimationFrame(function() {
                                            overflowWrapper.style.maxHeight = '0';
                                            eleRoot.parentNode.className = eleRoot.parentNode.className.replace('audentio_grid--expanded', 'audentio_grid--collapsed');
                                            ag.setCollapseCookie(eleRoot.eleID, false);
                                            window.setTimeout(function() {
                                                window.requestAnimationFrame(function() {
                                                    overflowWrapper.style.transition = 'none';
                                                    window.requestAnimationFrame(function() {
    ...
    initCollapseCookie: function() {
        var cookieVal = $.getCookie('uix_gridCollapsed');
        if (cookieVal == null) {
            audentio.grid.cookieArray = [];
        } else {
            audentio.grid.cookieArray = cookieVal.split(',');
        }
    },

    setCollapseCookie: function(nodeId, state) {
        nodeId = String(nodeId);
        var newArr = [];
        var newArrString = "";
        var found = false;
        for (var i = 0, len = audentio.grid.cookieArray.length; i < len; i++) {
            if (audentio.grid.cookieArray[i] !== nodeId) {
                newArr.push(audentio.grid.cookieArray[i])
            } else {
                found = true;
            }
        }
        ...
        for (var i = 0, len = newArr.length; i < len; i++) {
            if (newArrString = "") {
                newArrString = newArr[i];
            } else {
                newArrString = newArrString + ',' + newArr[i];
            }
        }

        audentio.grid.cookieArray = newArr;
        $.setCookie('uix_gridCollapsed', newArrString, new Date(new Date().setYear(new Date().getFullYear() + 5)));
    },

    getCollapseCookie: function(nodeId) {
        if (audentio.grid.cookieArray.indexOf(String(nodeId)) > -1) {
            return true; // forced expanded
        }
        return false;
    }
};
...
uix.sticky = {
	...
	scrollDetectorRunning: true, // boolean if the scrollDetector is running
			...
			if (currentItem.enabled) {
				...
				if (us.scrollDetectorRunning && currentItem.scrollSticky == 1) {
					...
					if (currentItem.state == 1) { //Is stuck
						...
						if (wrapperFromWindowTopInit - currentStickyHeight <= 0 && us.scrollDirection == 'up' && us.scrollDistance > us.stickyMinDist && us.scrollTop > us.stickyMinPos && !uix.offcanvas.triggerLeft && !uix.offcanvas.triggerRight) {
					...
					if (currentItem.state == 1) { //Is stuck
						...
						if ((wrapperFromWindowTopInit - currentStickyHeight <= 0) && (uix.scrollTop > 0 || !uix.preventAlwaysSticky) && !uix.offcanvas.triggerLeft && !uix.offcanvas.triggerRight) {
							us.stick(x, currentStickyHeight);
							needsPostStick = true;
...
uix.sidebarSticky = {
	...
	initGet: function() {
        if (uix.stickySidebarDisableIOS && uix.userAgent.match(/(iPod|iPhone|iPad)/)) {
            uix.stickySidebar = 0;
            return false;
        }

        uix.sidebarSticky.needsInit = false;
    ...
    resizeCheck: function() {
        ...
        if (us.running && uix.initDone && uix.sidebar.visible == 1 && !uix.offcanvas.triggerLeft && !uix.offcanvas.triggerRight) {
            var documentHeight,
                sidebarHeight,
            ...
            if (typeof(d.height) !== 'undefined') {
                documentHeight = d.height; // For webkit browsers
            } else {
	...
	resizeGet: function() {
		var us = uix.sidebarSticky,
			pfTime = uix.time();
		if (uix.elm.sidebar.length && uix.stickySidebar && uix.sidebar.visible == 1 && !uix.offcanvas.triggerLeft && !uix.offcanvas.triggerRight) {
			if (uix.windowWidth > uix.sidebarMaxResponsiveWidth) {
				uix.sidebarSticky.running = true;
	...
	updateGet: function() {
		...
		if (us.hasSidebar && uix.stickySidebar && us.running && !uix.offcanvas.triggerLeft && !uix.offcanvas.triggerRight) {
	...
	check: function() {
		...
		if (us.hasSidebar && uix.stickySidebar && uix.windowWidth > uix.sidebarMaxResponsiveWidth && !uix.offcanvas.triggerLeft && !uix.offcanvas.triggerRight) {
			var pfTime = uix.time();
			if (us.mainContainerHeight > us.sidebarHeight) {
                var topTarget = 0;
                var topDirectionSwitchOffset = 0; // used to prevent a jump when switching directions
                var sidebarExcess = us.sidebarHeight + uix.sticky.stickyHeight - uix.windowHeight;

                if (sidebarExcess > 0) {
                    uix.sidebarSticky.carryOverOffset = sidebarExcess + uix.globalPadding * 2 - uix.sticky.scrollDistance
                    if (uix.sidebarSticky.carryOverOffset < 0) {
                        uix.sidebarSticky.carryOverOffset = 0;
                    }

                    if (uix.sticky.scrollDirection == 'up' || uix.sticky.scrollDirection == 'start') {
                        topTarget = uix.scrollTop - us.sidebarOffset.top + uix.sticky.stickyHeight + uix.globalPadding;

                        /*
                        if (us.lastDirection == 'down') {
                            if (us.lastScrollDistance < sidebarExcess + uix.globalPadding * 2) {
                                us.scrollLockDistance = sidebarExcess + uix.globalPadding * 2 - us.lastScrollDistance;
                            } else {
                                us.scrollLockDistance = 0;
                            }
                        }

                        var scrollLockOffset = us.scrollLockDistance;
                        if (scrollLockOffset < 0) {
                            scrollLockOffset = 0;
                        }
                        */

                        topDirectionSwitchOffset = uix.sticky.scrollDistance - sidebarExcess - uix.globalPadding * 2;// + scrollLockOffset;

                        if (topDirectionSwitchOffset > 0) {
                            topDirectionSwitchOffset = 0;
                        }



                        uix.sidebarSticky.carryOverOffset = uix.sidebarSticky.carryOverOffset * -1;
                    } else {
                        topTarget = uix.scrollTop - us.sidebarOffset.top - us.sidebarHeight + uix.windowHeight - uix.globalPadding;

                        /*
                        if (us.lastDirection == 'up') {
                            if (us.lastScrollDistance < sidebarExcess + uix.globalPadding * 2) {
                                us.scrollLockDistance = sidebarExcess + uix.globalPadding * 2 - us.lastScrollDistance;
                            } else {
                                us.scrollLockDistance = 0;
                        ...
                        */
                        ...
                        topDirectionSwitchOffset = sidebarExcess - uix.sticky.scrollDistance + uix.globalPadding * 2;// - scrollLockOffset;
                        if (topDirectionSwitchOffset < 0) {
                            topDirectionSwitchOffset = 0;
                        }
                    ...
                    //console.log(uix.sidebarSticky.carryOverOffset);
                } else {
                    topTarget = uix.scrollTop - us.sidebarOffset.top + uix.sticky.stickyHeight + uix.globalPadding;
                ...
                topTarget += topDirectionSwitchOffset;
                ...
                if (topTarget <= 0) {
                    ...
                    us.stick(topTarget, topDirectionSwitchOffset, sidebarExcess, uix.sticky.scrollDirection);
                }

                uix.sidebarSticky.lastDirection = uix.sticky.scrollDirection;
                uix.sidebarSticky.lastScrollDistance = uix.sticky.scrollDistance;
	...
	stick: function(topTarget, switchOffset, sidebarExcess, scrollDirection) {
		var us = uix.sidebarSticky;

		if (us.hasSidebar && uix.stickySidebar && uix.windowWidth > uix.sidebarMaxResponsiveWidth && uix.sidebar.visible == 1) {
            uix.stickySidebar.lastTopPosition = topTarget;
			var pfTime = uix.time();
            ...
            var positionFixed = (switchOffset <= 0 && scrollDirection == 'down') || (switchOffset >= 0 && scrollDirection == 'up');
            if (positionFixed && us.allowFixed && topTarget < us.maxTop) {

    			uix.sidebarSticky.state = 2;
                var fixedTopTarget = uix.sticky.stickyHeight + uix.globalPadding;

                if (scrollDirection != 'up' && sidebarExcess > 0) {
                    fixedTopTarget -= (sidebarExcess + uix.globalPadding * 2);
                }
                uix.elm.sidebarInnerWrapper.css({
    				top: fixedTopTarget,
                ...
                if (topTarget > us.maxTop) {
                    topTarget = us.maxTop;
                }

    			uix.sidebarSticky.state = 1;

                if (us.lastTopPosition != topTarget) {
                    uix.elm.sidebarInnerWrapper.css({
                        top: topTarget,
        				left: '',
                        position: '',
	...
	unstick: function() {
		...
		if (us.hasSidebar && uix.stickySidebar && us.running) {
            uix.stickySidebar.lastTopPosition = -1;
...
uix.swfupload = {
	...
	init: function() {
		uix.swfupload.initGet();
		uix.swfupload.initSet();
	},

	initGet: function() {
		var ele = document.querySelectorAll('#ctrl_uploader'),
			target = document.querySelectorAll('#SWFUpload_0');
		if (ele.length && target.length) {
			uix.swfupload.ele = ele[0];
			uix.swfupload.target = target[0];
			uix.swfupload.enabled = true;
		}
	},

	initSet: function() {
		uix.swfupload.needsInit = false;
	},
	...
	resizeGet: function() {
		if (uix.swfupload.enabled) {
			uix.swfupload.width = uix.swfupload.ele.offsetWidth;
		}
	},

	resizeSet: function() {
		var us = uix.swfupload;
		if (us.enabled) {
			if (us.width != us.lastWidth) {
				uix.swfupload.lastWidth = us.width;
				us.target.style.width = us.width + 'px';
			}
		}
	}
...
uix.toggleWidth = {
	...
	setup: function() {
		if (XenForo.visitor.user_id) {
			...
			uix.toggleWidth.state = $.getCookie('uixWidthToggle');
			if (uix.toggleWidth.state === null) {
				uix.toggleWidth.state = uix.user.widthToggleState;
			}

			if (parseInt(uix.toggleWidth.state, 10) === 1) {
				$('html').removeClass('is-setWidth').addClass('is-fullWidth');
				uix.toggleWidth.setIcon(1);
			}
		}

		window.setTimeout(function() {
			$('html').addClass('uix_toggleWidth--init'); // adds transition to width change, delayed for guest
		}, 100);
	...
	toggle: function() {
		...
		if (XenForo.visitor.user_id) {
			...
			$.setCookie('uixWidthToggle', uix.toggleWidth.state)
		}
	}
};
...
$(document).ready(function() {
	$('a.ReplyQuote, a.QuoteSelected').click(function() {
        var quickEle = $('#QuickReply');

        if (quickEle.length) {
            var quickEleTop = quickEle.offset().top;
            $(document).scrollTop(quickEleTop - uix.sticky.getOffset(quickEleTop)); // modified line
        }
	});
});


var xenforoQuickReply = XenForo.QuickReply;
...
XenForo.QuickReply = function($form) {
	...
	this.scrollAndFocus = function() {
        if ($form.length) {
            ...
            var formEleOffset = $form.offset().top;
            $(document).scrollTop(formEleOffset - uix.sticky.getOffset(formEleOffset)); // modified line
        }
...
XenForo.PopupMenu.prototype.controlClick = function(e) {
	...
	if (!this.newlyOpened && (true || !this.$menu.is(':animated'))) {
...
XenForo.PopupMenu.prototype.menuShown = function()
{
    ...
    var childAnchor = this.$control.find('.navLink');

	this.newlyOpened = true;
    if (childAnchor.length == 1 && childAnchor.hasClass('uix_dropdownDesktopMenu')) {
        setTimeout($.context(function()
    	{
    		this.newlyOpened = false;
    	}, this), 300); // slightly longer delay
    } else {
        setTimeout($.context(function()
    	{
    		this.newlyOpened = false;
	...
	if ($balloon.length) {
		...
		if (!newTotal || newTotal == '0') {
			...
			$balloon.fadeIn('fast', function() {
				...
				var newTotalInt = parseInt(newTotal.replace(/[^\d]/, ''), 10);
				var newDifference = newTotalInt - oldTotalInt;
...
// #																					 #
// #######################################################################################
// Override default xenforo function to leave more space so prefix is on same line as title.
...
XenForo.TitlePrefix = function($select) {
	...
	function setTextboxWidth(e) {
		if ($textbox.length) {
			var w = $wrap.innerWidth() - 40; // overwrite this line
			...
			if (!xhr)
			{
				...
				(
					...
					function(ajaxData, textStatus)
					{
						...
						else if (typeof (ed.val) === 'function')
						{
...
uix.fn.scrollToAnchor = function(href) {
	...
	if (href.indexOf('#') !== -1) {
		...
		if (split.length == 2 && split[1].length > 0) {
			...
			//var $target = $('a[href=\'' + href + '\']'); // KC -- this caused a bug
            var hashId = split[1].replace('.', '\\.');
            if (hashId.indexOf('=') === -1) {
                var $target = $('#' + hashId);
    			if ($target.length) {
...
uix.hideDropdowns = {
	...
	hide: function() {
		...
		if (uix.hideDropdowns.eles !== null) {
			...
			var openedPopups = $('.PopupOpen').removeClass('PopupOpen').addClass('.PopupClosed')
		}
	}
};

uix.javascriptFinished = function() {
...
uix.fixScrollLocation = {
	...
	initGet: function() {
		...
		if (hash.length > 2) {
			var pfTime = uix.time();
			hash = hash.replace('/', '').split('.').join('\\.'); // a slash breaks jQuery as do periods
			console.log(hash);
			var $target = $(hash);
			if ($target.length) {
				uix.fixScrollLocation.topOffset = $target.offset().top;
            } else {
                uix.fixScrollLocation.topOffset = -1;
            }
...
uix.ajaxSuccessResize = function() {
    $(document).on('XFAjaxSuccess', function(){
        ...
        if (uix.isSet(activeEle)) {
            ...
            } else if (activeEle.tagName === 'INPUT' || activeEle.tagName === 'TEXTAREA') {
                resizeAllowed = false;
        ...
        if (resizeAllowed) {
            window.setTimeout(function(){
                //uix.resizeFire() // KC commented out for now, causing too many issues
            }, 0);
        }
    });
}

uix.jsVersion = '1.5.14.0_public';
...
uix.initFunc = function() {
	...
	uix.initGet = function() {
		...
		uix.logI('Pre UIX SWFUpload Init Get');
		uix.swfupload.initGet();
	};
	...
	uix.initSet = function() {
		...
		uix.logI('Pre UIX SWFUpload Init Set');
		uix.swfupload.initSet();
		...
		if (uix.elm.sidebar.length && uix.stickySidebar && uix.windowWidth > uix.sidebarMaxResponsiveWidth && uix.sidebar.visible == 1 && !uix.stickyForceDisable && uix.user.stickyEnableSidebar) {
			if (uix.stickySidebarDelayInit) {
				$(window).load(function() {
                    if (uix.sidebarSticky.needsInit) {
                        window.setTimeout(function() {
                            uix.sidebarSticky.update();
                        }, 6000)
                    }
